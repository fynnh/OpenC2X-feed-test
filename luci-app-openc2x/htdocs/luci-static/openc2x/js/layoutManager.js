...
function createMessageLogger(){
	if($("#logger").length === 0){ //container is not created
		var htmlstr = 
		'<fieldset id="logger" class="container dataContainer cbi-section" draggable="true"> '+
			'<div class="openc2x-content">' +
				'<a href="javascript:void(0)" class="close" onclick="closeThisContainer(logger)"><span style="float: right;">x</span></a>'+
				'<span class="panel-title"> Log messages</span>'+
				'<div class="cbi-tabcontainer" id="loggerMessages">' +
				'</div>' +
				'<input placeholder="Type message here and enter" class="cbi-input-text" id="loggerInput">'+
        	'</div>' +
    	'</fieldset>';
	$("#openc2x-grid").append(htmlstr);
	
	$("#loggerInput").keyup(function(event) {
		console.log("a");
		if (event.keyCode === 13) {
			console.log($("#loggerInput"));
			text = $("#loggerInput").val();
			sendMessage(text, function(datetime) {
				$("#loggerInput").val("");
				var insert = $('<div class="cbi-value"></div>');
				insert.text(datetime + ": " +text);
				$("#loggerMessages").append(insert);	
				
				$('#loggerMessages').animate({ // animate your right div
					scrollTop: insert.position().top // to the position of the target 
				}, 400);	
			});
...
function createDenmContainer(){
	...
	containers.push(new Container("Denm", function(callback) {
			denmData.getLastOwnDenm(callback);
		},color= "#FFFF00"));
	}
}
